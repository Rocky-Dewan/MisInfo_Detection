...
DATASETS = [
    {"id":"mhmisinfo","name":"MHMisinfo","goal":"Misinformation Detection and Public Engagement","size":"739 videos","rows":739,"cols":12,"description":"A curated benchmark of YouTube videos annotated for mental-health misinformation. Each sample includes title, description, tags, auto-generated transcript (audio & video), engagement statistics, and a binary credibility label."},
    {"id":"yt_full","name":"YT Full Dec-16","goal":"Extended YouTube social-media corpus with metadata, transcription, and engagement signals","size":"640 videos","rows":640,"cols":22,"description":"A large-scale YouTube data dump collected in December 2016, enriched with metadata and auto-transcription. Used as the primary training corpus with diverse health-topic videos and view, like, and comment statistics."},
]
...
def page_dataset_analysis():
    st.markdown('<div style="margin-bottom:28px"><div class="section-label" style="margin-bottom:8px">📊 Benchmark Results</div><h2 style="margin:0;font-size:1.8rem;font-weight:800;letter-spacing:-0.02em">Dataset Analysis</h2><p style="color:var(--text-mid);margin:8px 0 0;font-size:0.95rem">Select a dataset to explore its metrics, training progression, and model performance.</p></div>', unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
    ...
    selected_ds_id = st.session_state.get("selected_ds", None)
    st.markdown('<div class="ds-table-header"><span>Dataset</span><span>Goal</span><span>Size</span><span>Action</span></div>', unsafe_allow_html=True)

    for ds in DATASETS:
        is_sel = selected_ds_id == ds["id"]
        active_badge = "<span style='font-size:0.7rem;background:var(--emerald-light);color:var(--emerald);padding:2px 8px;border-radius:20px;font-weight:600'>Active</span>" if is_sel else ""
        st.markdown(f'<div class="ds-row {"selected" if is_sel else ""}"><div><div class="ds-name">{ds["name"]}</div>{active_badge}</div><div class="ds-goal">{ds["goal"]}</div><div class="ds-size">{ds["size"]}</div><div></div></div>', unsafe_allow_html=True)
        if st.button("✓ Selected" if is_sel else "Select →", key=f"btn_{ds['id']}", disabled=is_sel):
            st.session_state["selected_ds"] = ds["id"]
            st.rerun()

    if selected_ds_id is None:
        st.markdown('<div style="text-align:center;padding:48px;color:var(--text-light)"><div style="font-size:3rem;margin-bottom:12px">☝️</div><div style="font-weight:600;font-size:1rem">Select a dataset above to view its analysis</div></div>', unsafe_allow_html=True)
        return

    ds = next(d for d in DATASETS if d["id"] == selected_ds_id)
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown(f'<div style="margin-bottom:22px"><div class="section-label" style="margin-bottom:6px">Results for</div><h3 style="margin:0;font-size:1.4rem;font-weight:800">{ds["name"]}</h3></div>', unsafe_allow_html=True)

    col_bars, col_info = st.columns([1.4, 1], gap="large")
    with col_bars:
        st.markdown('<div class="section-label" style="margin-bottom:14px">📏 Performance Metrics</div>', unsafe_allow_html=True)
    ...
    with col_info:
        st.markdown('<div class="section-label" style="margin-bottom:14px">🗂️ Dataset Info</div>', unsafe_allow_html=True)
        st.markdown(stat_grid_html([(str(ds["cols"]),"Total Columns"),]), unsafe_allow_html=True)
        st.markdown(f'<div class="vcard-mini" style="margin-bottom:14px"><div class="section-label" style="margin-bottom:8px">📝 Description</div><p style="font-size:0.84rem;color:var(--text-mid);line-height:1.65;margin:0">{ds["description"]}</p></div>', unsafe_allow_html=True)
        st.markdown("""
<div class="vcard-mini"><div class="section-label" style="margin-bottom:12px">🔢 Confusion Matrix</div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
    ...
    st.markdown('<div class="vcard" style="height:auto"><div class="section-label" style="margin-bottom:14px">⚙️ Training Config</div>', unsafe_allow_html=True)
    ...
    for k, v in config_items:
        st.markdown(
            f'<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid var(--border)">'
            f'<span style="font-size:0.82rem;color:var(--text-mid)">{k}</span>'
            f'<span style="font-size:0.82rem;font-weight:600;font-family:\'DM Mono\',monospace;color:var(--text-dark)">{v}</span>'
            f'</div>', 
            unsafe_allow_html=True
        )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    # --- VISUAL ANALYSIS ---
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<div class="section-label" style="margin-bottom:14px">📊 Visual Analysis</div>', unsafe_allow_html=True)
    ...
    with pc1:
        ...
        values = [REPORT_METRICS[k] for k in ["accuracy", "precision_macro", "recall_macro", "f1_macro", "roc_auc", "pr_auc"]]
    ...
    with pc2:
        ...
        loss_curve = [max(0.05, 0.9 - 0.07 * i + random.uniform(-0.03, 0.03)) for i in range(REPORT_CONFIG["epochs"])]
        
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=epochs_x, y=[v*100 for v in acc_curve], name="Accuracy (%)", mode="lines+markers", line=dict(color="#10B981", width=2.5), marker=dict(size=6)))
        fig2.add_trace(go.Scatter(x=epochs_x, y=[v*100 for v in loss_curve], name="Loss (scaled)", mode="lines+markers", line=dict(color="#EF4444", width=2.5, dash="dot"), marker=dict(size=6), yaxis="y2"))
...
# ─ Tiny HTML helpers (inlined — never depend on broken external functions) 

def _metric_card(icon: str, value: str, label: str, border_color: str) -> str:
    """Return a self-contained metric card as an HTML string."""
    return (
...
def _stat_card(emoji: str, pct: float, label: str, color: str) -> str:
    """Return a sentiment stat card as an HTML string."""
    return (
        f'<div style="background:#13161e;border:1px solid #1e2330;border-radius:12px;'
...
def _run_analysis_pipeline(vid_id, api_key, max_comments, sentiment_method, fetch_tr):
    """
    Full analysis pipeline used by both the URL tab and the Search tab.
    All rendering uses inline HTML — no external metric_card_html() / video_info_grid_html().
    """
    from src import charts as ch
...
<div style="background:linear-gradient(135deg,{banner_color}22,{banner_color}11);
  ...
  <div style="font-size:2.2rem;font-weight:700;color:{banner_color};font-family:monospace;margin-top:4px">{conf}% Confidence</div>
    ...
    #  7. Modality analysis (3 charts) ─
    st.markdown("#### 🔬 Modality Analysis")
    mod_analysis = result.get("modality_analysis", {})
    col1, col2, col3 = st.columns(3)
    with col1:
        st.plotly_chart(
            ch.modality_misinfo_distribution(mod_analysis),
            use_container_width=True, config={"displayModeBar": False},
        )
    with col2:
        st.plotly_chart(
            ch.trust_score_by_modality(mod_analysis),
            use_container_width=True, config={"displayModeBar": False},
        )
    with col3:
        st.plotly_chart(
            ch.uncertainty_analysis(mod_analysis),
            use_container_width=True, config={"displayModeBar": False},
        )
    ...
    if not comments_df.empty:
        ...
        # NOTE: Avoid one-liner `st.x() if cond else st.y()` — the return value
        # is a DeltaGenerator, which Streamlit would display as garbage text if
        # the expression result is somehow captured.  Use explicit if/else blocks.
        tab_pos, tab_neg, tab_neu = st.tabs(
            ["Positive Engagement", "Negative Engagement", "Neutral"]
        )
...
""", unsafe_allow_html=True)
    ...
    # API key from env — no text_input field
    api_key = YT_API_KEY
    if not api_key:
    ...
    tab_url, tab_upload = st.tabs(
        ["🔗 YouTube URL / Video ID", "📁 Upload Video / Search by Title"]
    )

    #  URL tab 
    with tab_url:
        # Wrap in st.form so pressing Enter triggers the submit button
        with st.form(key="video_lab_url_form", clear_on_submit=False):
            ...
            with st.expander("⚙️ Advanced Options", expanded=False):
                ...
                sentiment_method = st.radio(
                    "Sentiment method", ["vader", "hf"], horizontal=True,
            ...
            run_btn = st.form_submit_button(
                "🔍 Analyse", type="primary", use_container_width=False
            )
            ...
            if not yt_url.strip():
                ...
                if not vid_id:
                    ...
                    _run_analysis_pipeline(
                        vid_id, api_key, max_comments, sentiment_method, fetch_tr
                    )
    ...
    with tab_upload:
        ...
        with col_search:
            search_clicked = st.button(
                "🔎 Find on YouTube", use_container_width=True, key="search_btn"
            )
        ...
        if search_clicked and keyword.strip():
            ...
            if not results:
                ...
                if st.button("▶ Analyze Selected Video", type="primary", key="analyze_selected"):
                    _run_analysis_pipeline(
                        choices[selected_title], api_key,
...
# ─ ROUTING (paste this at the bottom of your main app.py) 
#
...
st.markdown(
    ...
    'Misinformation Detection and Publich Engagement · Built on '
